...
void ConvertSQ( void ) {
  ...
  for( c=0; c<256; c++ ) {
    // The cmix byte model historically gave every vocabulary byte at least
    // one count before normalizing. Build the binary tree with the same
    // smoothing here, so bit predictions can use trF/trT directly without
    // rescanning byte ranges on every bit.
    const uint mass = sqp[c] ? sqp[c] : 1;
    for( i=8; i!=0; i-- ) {
      j = (256+c)>>i;
      b = (c>>(i-1))&1;
      trF[j]+= (b == 0) * mass;
